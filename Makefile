...
.PHONY: test shellcheck test-render test-launchers test-devcontainer help

help:
	@echo "make test               Run all checks (shellcheck + render + launchers + devcontainer)."
	...
	@echo "make test-launchers     Assert the host-side bin/ launchers resolve the root without git (fast, no Docker)."
...
test: shellcheck test-render test-launchers test-devcontainer

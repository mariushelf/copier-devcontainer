...
| Component     | Details                                                                                                                                                                                                                                                                                                                                                                             |
|---------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Base image    | `ubuntu:26.04` (LTS — ships git 2.53 for relative worktrees)                                                                                                                                                                                                                                                                                                                                                              |
...
2. work in git worktrees inside the container, allowing for multiple parallel agents
   ...
   agents. The image enables `worktree.useRelativePaths` system-wide (git config), so
   worktrees created in the container use relative paths and keep working even when the
   bind-mounted repo lives at a different path on the host.
...
in `init-firewall.sh`. [How-tos](HOWTOS.md#updating-the-firewall-allow-list)
covers applying the change to a running or restarted container.
...
**Known limitation:** the allow list is enforced at the IP layer, so it cannot
distinguish domains that share a CDN address with an allowed host (Fastly,
GitHub Pages), and IPv6 egress is currently unfiltered. The posture is adequate
for the threat model this container targets but is not exfiltration-proof.
...
step 2). [How-tos](HOWTOS.md#rotating-the-github-token) covers replacing
the token later.
